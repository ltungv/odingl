...
main :: proc() {
  ...
  window, is_ok = commons.glfw_window_create(800, 600, "coordinates")
  if !is_ok do return
  ...
  shader_program, is_ok = commons.gl_load_source(
    string(#load("coordinates.vert.glsl")),
    string(#load("coordinates.frag.glsl")));
  if !is_ok do return
  ...
  // Move the scene in the negative z-direction, which makes the scene moves further
  // away from the camera
  view := linalg.matrix4_translate(linalg.Vector3f32{0.0, 0.0, -3.0})
  // Make vertices that are further away smaller, with
  // + a field-of-view (FOV) of 45 degrees
  // + 4/3 aspect ratio
  // + 100.0 units of distance between the front and back panels
  proj := linalg.matrix4_perspective(linalg.radians(f32(45.0)), 4.0 / 3.0, 0.1, 100.0)
